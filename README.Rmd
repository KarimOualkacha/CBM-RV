...
This an R package which implements a Multivariate Copula-Based Rare-Varaints (CBMRV) region-based association test, in presence of familial data. 
...
The CBMRV package is under construction, however, source R functions that are needed to calculate the score test statistics and p.vlaues can be obtained from the GitHub repository ```CBMRV```. To run the CBMRV test, one can download the CBMRV R source files to a personal directory and call all R functions using ```source()``` function in R. See below a toy example for more details on how to calculate p.values. 
...
## Running example
All the required R functions, including the two main R functions ```CMB.RV``` and ```CMB.RV.null```, can be downloaded via ```source()``` funtion in R, as follow:

``` r
path = "path_to_R_files_"
